<template>
    <div class="pages pmcplist">
        <div class="sline">
            <div class="sitem" v-for="item in types" v-bind:key="item.type" @click="changetype(item)"><span :class="{'skin':true,'active gb-c-bdc':type_ac==item.type}">{{item.name}}</span></div>
        </div>
        <div class="cplist">
            <div class="item" v-for="item in list" v-bind:key="item.id">
                <div class="itop">
                    <div :class="['item-l',{'disable':item.type==1||item.type==2},,{'gb-c':item.type!=1&&item.type!=2}]">
                        <div class="item-lt"><span class="item-lu">¥</span>{{item.money}}</div>
                        <div class="item-ru">{{item.rule}}</div>
                    </div>
                    <div :class="['item-r',{'disable':item.type==1||item.type==2},,{'gb-c':item.type!=1&&item.type!=2}]">
                    ...
                    <img v-if="item.type==2" src="~@/assets/images/coupon_use.png" class="item-biao" />
                </div>
                <router-link v-if="item.type==0" class="item-link gb-c" to="/category/list">去使用</router-link>
            </div>
        </div>
...
<style lang="less">
    .pmcplist{
        ...
        .item{ margin-bottom: .1rem; background: #fff; border-radius: .08rem; color:@base; border-color: @base;}
        ...
        .item-link{ display: block; line-height: .4rem; color: @base; text-align: center; border-top: 1px dashed #eee;}
        .item-biao{ position: absolute;right: 0; bottom: 0; width: .54rem;}
        .disable{color: #999; border-color:#999;}
